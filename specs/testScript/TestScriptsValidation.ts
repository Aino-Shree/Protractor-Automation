...
function testScripts() {
  ...
  browser.getCurrentUrl().then(function (text: string) {
    expect(text.toString()).toBe(browser.params.baseUrl + '#' + 'testScripts');
...
function newScripts(Name) {
  ...
  browser.driver.sleep(5000);
  browser.getCurrentUrl().then(function (text: string) {
    expect(text.toString()).toBe(browser.params.baseUrl + '#newScripts');
...
function inUse(scriptName) {
  ...
  browser.getCurrentUrl().then(function (text: string) {
    expect(text.toString()).toBe(browser.params.baseUrl + '#' + 'scriptInUse');
  ...
  browser.getCurrentUrl().then(function (text: string) {
    expect(text.toString()).toBe(browser.params.baseUrl + '#' + 'editScript');
    browser.driver.sleep(5000);
...
function inDraft(scriptName) {
  ...
  browser.driver.sleep(3000);
  browser.getCurrentUrl().then(function (text: string) {
    expect(text.toString()).toBe(browser.params.baseUrl + '#' + 'scriptInDraft');
    console.log("---In Use script window is opened--- ");
    browser.driver.sleep(5000);
